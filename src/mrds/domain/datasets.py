...
class DatasetMetadata(MRDSDomainModel):
    ...
    author: Optional[str] = Field(default=None, description="Author or team who created the dataset.")
    description: Optional[str] = Field(default=None, description="Description of the dataset's purpose.")
...
class DatasetCaseSchema(MRDSDomainModel):
    ...
    difficulty: DifficultyLevel = Field(default=DifficultyLevel.MEDIUM, description="Difficulty of the case.")
    variables: Dict[str, Any] = Field(..., description="Variables to inject into the prompt template.")
    expected_output: Optional[str] = Field(default=None, description="The exact expected output, if applicable.")
    evaluation_criteria: List[str] = Field(
        default_factory=list, 
        description="List of criteria for an LLM judge to evaluate this case."
    )
...
class DatasetSchema(MRDSDomainModel):
    ...
    cases: List[DatasetCaseSchema] = Field(..., min_length=1, description="List of test cases in this dataset.")
